...
public class GeneticAlgorithmSelAtt<A> {
	...
	protected List<Individual<A>> nextGeneration(List<Individual<A>> population, FitnessFunction<A> fitnessFn) {
		...
		// for i = 1 to SIZE(population) do
                newPopulation.add(new Individual<>(retrieveBestIndividual(population,fitnessFn).getRepresentation())); //Añade el mejor individuo de la población vieja
		for (int i = 1; i < population.size(); i++) {
                    Individual<A> child;
                    Individual<A> worstIndividual = new Individual<>(retrieveWorstIndividual(population,fitnessFn).getRepresentation());
                    //ACEPTACIÓN DE MEJORA - Solo lo añadimos a la siguiente población
                    //si el individuo es mejor que el peor de la población.
                    //Por tanto si no genera uno mejor que el peor, se vuelve
                    //a generar el individuo para mantener el tamaño de la poblacion
                    do{
			// x <- RANDOM-SELECTION(population, FITNESS-FN)
			...
			if (random.nextDouble() <= mutationProbability) {
				child = mutate(child);
                    ...
                    }while(fitnessFn.apply(child)<fitnessFn.apply(worstIndividual)); //Mientras el fitness del individuo generado sea peor que el peor de la poblacion
	...
	protected Individual<A> mutate(Individual<A> child) {
		int mutateOffset = randomOffset(2);
		int alphaOffset = randomOffset(2);
